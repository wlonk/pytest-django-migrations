    ...
    def run_executor(self, target):
        with transaction.atomic():
            ...
            post_migrate_state = executor.migrate(
                target, plan=plan, state=pre_migrate_state.clone()
            )
    ...
    def to_migration(self, migrate_to):
        def inner_decorator(fn):
            ...
            def wrapper(*args, **kwargs):
                ...
                with transaction.atomic():
                    return fn(*args, old_apps=old_apps, apps=apps, **kwargs)
...
def setup_migration(app, migrate_from):
    """
    Use::

        @pytedjmi.setup_migration("app", "starting point")
        def some_setup(apps):
            pass
